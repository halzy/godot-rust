...
pub unsafe fn {cname}_{name}(obj_ptr: *mut sys::godot_object{params}) -> {rust_ret_type} {{
    ...
    let method_bind: *mut sys::godot_method_bind = crate::{namespace}{table_name}.as_ref().unwrap().{cname}__{name};"#,
        namespace = namespace,
...
pub fn generate_methods(
    ...
    pub unsafe fn {rusty_name}({self_param}{params_decl}) -> {rust_ret_type} {{
        "#,
            ...
            generate_method_body(output, table_name, namespace, class, method)?;
            writeln!(
                output,
                r#"
    }}"#
...
fn generate_method_body(
    ...
    if method.has_varargs {
        ...
        for argument in &method.arguments {
            let ty = argument.get_type().to_rust().unwrap();
            if ty.starts_with("Option") {
                // FIXME: check that this is correct, not seeing where it is used
                writeln!(
                    output,
                    r#"
        ...
        writeln!(
            ...
            let ret = Variant::from_sys((gd_api.godot_method_bind_call)(method_bind, self.this, argument_buffer.as_mut_ptr(), argument_buffer.len() as _, ptr::null_mut()));"#
        )?;
